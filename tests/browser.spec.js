...
test('admin dashboard, management, inventory and responsive layout', async ({
  page,
}) => {
  await page.goto('/');
  await expect(
  ...
  await expect(
    page.getByRole('cell', { name: 'Jubilee Hills', exact: true }).first(),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole('cell', { name: 'Jubilee Hills', exact: true }),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Inventory', exact: true }).click();
  await expect(
    page.getByRole('heading', { name: 'Ingredient inventory' }),
  ).toBeVisible();
  await expect(
    page.getByRole('cell', { name: 'Rice', exact: true }).first(),
  ).toBeVisible();
  await page.screenshot({ path: 'screenshots/inventory.png', fullPage: true });
  await page.getByRole('button', { name: 'Reports', exact: true }).click();
  await expect(
    page.getByRole('heading', { name: 'Customer behavior' }),
  ).toBeVisible();
...
test('staff creates and pays an order; customer submits receipt feedback', async ({
  page,
  context,
}) => {
  ...
  await expect(
    page.getByRole('status').filter({ hasText: 'Order created' }),
  ).toBeVisible();
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole('status').filter({ hasText: 'Payment recorded' }),
  ).toBeVisible();
  await page.getByRole('button', { name: 'View', exact: true }).first().click();
  const href = await page
    .getByRole('link', { name: 'Open customer feedback page' })
    .getAttribute('href');
