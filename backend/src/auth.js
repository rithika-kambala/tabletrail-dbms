...
export async function authenticate(req, res, next) {
  ...
  const [user] = await query(
    'SELECT id,name,email,role,branch_id FROM employees WHERE id=? AND active=TRUE',
    [payload.sub],
  );
...
export const roles =
  ...
  (req, res, next) => {
    if (!allowed.includes(req.user.role))
      throw fail(403, 'Your role cannot perform this action');
    next();
  };
export function branchAccess(user, branch) {
  if (user.role !== 'admin' && Number(branch) !== user.branch_id)
    throw fail(403, 'This branch is outside your access');
}

export function receiptToken(orderId) {
  return jwt.sign({ sub: String(orderId) }, secret, {
    expiresIn: '30d',
    algorithm: 'HS256',
    audience: 'feedback',
  });
...
export function verifyReceipt(token) {
  try {
    return Number(
      jwt.verify(token, secret, { algorithms: ['HS256'], audience: 'feedback' })
        .sub,
    );
  } catch {
    throw fail(400, 'This feedback link is invalid or expired');
  }
}
