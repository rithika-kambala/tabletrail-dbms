...
export const config = {
 ...
 user: process.env.DB_USER || 'tabletrail', password: process.env.DB_PASSWORD,
 database: process.env.DB_NAME || 'tabletrail', decimalNumbers: true,
};
export const pool = mysql.createPool({ ...config, connectionLimit: 10 });
export async function query(sql, values = [], db = pool) {
 const [rows] = await db.execute(sql, values); return rows;
}
